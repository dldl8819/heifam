  ...
  return (
    <section className="space-y-6">
      <div className="rounded-lg border border-cyan-100 bg-white px-5 py-6 shadow-sm sm:px-6">
        <Link href="/events" className="text-sm font-semibold text-cyan-700 hover:text-cyan-900">
          이벤트 목록
        </Link>
        <div className="mt-4 max-w-4xl space-y-3">
          ...
          <h2 className="text-3xl font-bold tracking-tight text-slate-950">{event.title}</h2>
          <p className="text-sm font-semibold text-slate-500">{event.dateLabel}</p>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,360px)]">
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-700">{event.teamAssignmentNote}</p>
          </section>
          ...
          {event.resultNote && (
            <section className="rounded-lg border border-emerald-200 bg-emerald-50 p-5">
              ...
              <p className="mt-3 text-sm leading-6 text-emerald-800">{event.resultNote}</p>
            </section>
          )}
        </aside>
      </div>
