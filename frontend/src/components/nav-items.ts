...
export function getVisibleNavItems(context: NavVisibilityContext): NavItem[] {
  ...
  if (context.isAdmin) {
    ...
    if (context.isSuperAdmin) {
      ...
      adminItems.splice(adminItems.length - 1, 0, { label: t('nav.matchHistory'), href: '/admin/matches' })
      adminItems.splice(adminItems.length - 1, 0, { label: t('nav.notices'), href: '/notices' })
    }
